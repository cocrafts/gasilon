...
export async function getPopularTokens(
	...
	const mints = response.data.map(
		(mint: string) => new PublicKey(mint),
	) as PublicKey[];
	const filteredMints = excludeNative
		? mints.filter((value) => !value.equals(NATIVE_MINT))
		: mints;
	return filteredMints.slice(0, count);
}

export async function getTokenToNativePriceInfo(
	mint: PublicKey,
): Promise<TokenPriceInfo> {
	const priceInfoResponse = (
		await axios.get('https://price.jup.ag/v1/price', {
			params: { id: 'SOL', vsToken: mint.toBase58() },
		})
...
export async function getRoutes(
	...
	const routesResponse = (
		await axios.get('https://quote-api.jup.ag/v1/quote', { params })
...
export async function getSwapTransactions(
	...
	const decodeTransactionOrNull = (serialized: string | null) =>
		serialized !== null
			? Transaction.from(Buffer.from(serialized, 'base64'))
			: null;
